...
private:
    ...
    void inorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
            ++melyseg;
            inorder (elem->nullasGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            inorder (elem->egyesGyermek (), os);
            --melyseg;
        }
    }
    void preorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
    ...
    void postorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
...
main (int argc, char *argv[])
{
    ...
    if (c.compare("-in") == 0)
    {
        binFa.inorder(kiFile);
    }
    else
    if (c.compare("-pre") == 0)
    {
        binFa.preorder(kiFile);
    }
    else
    if (c.compare("-post") == 0)
    {
        binFa.postorder(kiFile);
    }
    //std::cout << binFa.kiir (); // így rajzolt ki a fát a korábbi verziókban de, hogy izgalmasabb legyen
    // a példa, azaz ki lehessen tolni az LZWBinFa-t kimeneti csatornára:
