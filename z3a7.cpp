...
int egyes = 0;
   ...
   */
    void kiir_inorder (void)
    {
        ...
        kiir_inorder (&gyoker, std::cout);
    }

    void kiir_preorder (void)
    {
        // Sokkal elegánsabb lenne (és más, a bevezetésben nem kibontandó reentráns kérdések miatt is, mert
        // ugye ha most két helyről hívják meg az objektum ilyen függvényeit, tahát ha kétszer kezd futni az
        // objektum kiir() fgv.-e pl., az komoly hiba, mert elromlana a mélység... tehát a mostani megoldásunk
        ...
        // ha nem mondta meg a hívó az üzenetben, hogy hova írjuk ki a fát, akkor a
        // sztenderd out-ra nyomjuk
        kiir_preorder (&gyoker, std::cout);
    }

    void kiir_postorder (void)
    {
        // Sokkal elegánsabb lenne (és más, a bevezetésben nem kibontandó reentráns kérdések miatt is, mert
        // ugye ha most két helyről hívják meg az objektum ilyen függvényeit, tahát ha kétszer kezd futni az
        // objektum kiir() fgv.-e pl., az komoly hiba, mert elromlana a mélység... tehát a mostani megoldásunk
        ...
        // ha nem mondta meg a hívó az üzenetben, hogy hova írjuk ki a fát, akkor a
        // sztenderd out-ra nyomjuk
        kiir_postorder (&gyoker, std::cout);
    }
  ...
  */
    ...
    void kiir_inorder (std::ostream & os)
    {
        melyseg = 0;
        kiir_inorder (&gyoker, os);
    }

    void kiir_preorder (std::ostream & os)
    {
        melyseg = 0;
        kiir_preorder (&gyoker, os);
    }

    void kiir_postorder (std::ostream & os)
    {
        melyseg = 0;
        kiir_postorder (&gyoker, os);
    }
...
private:
    ...
    void kiir_inorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
            ++melyseg;
            kiir_inorder (elem->nullasGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            kiir_inorder (elem->egyesGyermek (), os);
            --melyseg;
        }
    }

    void kiir_preorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
            ...
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            kiir_preorder (elem->nullasGyermek (), os);
            kiir_preorder (elem->egyesGyermek (), os);
            --melyseg;
        }
    }

    void kiir_postorder (Csomopont * elem, std::ostream & os)
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)
        {
            ++melyseg;
            kiir_postorder (elem->nullasGyermek (), os);
            kiir_postorder (elem->egyesGyermek (), os);
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
...
usage (void)
{
    std::cout << "Usage: lzwtree in_file -o out_file -lol" << std::endl;
}
...
main (int argc, char *argv[])
{
    ...
    //std::cout << binFa.kiir (); // így rajzolt ki a fát a korábbi verziókban de, hogy izgalmasabb legyen
    // a példa, azaz ki lehessen tolni az LZWBinFa-t kimeneti csatornára:
   //Innen kivettük a kiFile.binfa		// ehhez kell a globális operator<< túlterhelése, lásd fentebb
    // (jó ez az OO, mert mi ugye nem igazán erre gondoltunk, amikor írtuk, mégis megy, hurrá)
   std::char c  = argv[1];

	if (c == 'p')
    {
    	binFa.kiir_preorder(kiFile);
    }
    else if (c == 't')
    {
    	binFa.kiir_postorder(kiFile);
    }
    else if (c == 'i')
    {
    	binFa.kiir_inorder(kiFile);
    }

    kiFile << "depth = " << binFa.getMelyseg () << std::endl;
    kiFile << "mean = " << binFa.getAtlag () << std::endl;
    kiFile << "var = " << binFa.getSzoras () << std::endl;
    kiFile << binFa.nulla <<std::endl;
    kiFile << binFa.egyes<<std::endl;
